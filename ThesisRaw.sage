		...
		for key in ModularSymbolsfile.keys():
			for prime in ModularSymbolsfile[key].keys():
				...
				if LeftSide % jajaj != RightSide % jajaj and LeftSide % jajaj != - RightSide % jajaj:
					file.write(cremona_to_lmfdb(str(EllipticCurveSage.cremona_label())) + ";" + str(jajaj) + ';' + str(LeftSide % good_primes) + ';' + str(RightSide % good_primes) + ';' + str(CremonaEllipticCurve['rank']) + ';' +'${}\\cdot{}^{} + O({}^{})$'.format(torsion, good_primes, meh, good_primes, meh + 1) + ';' + str(EllipticCurveSage.modular_degree()) +  '\n')
	...
	for i in ['00000-09999', '10000-19999', '20000-29999','30000-39999','40000-49999','50000-59999','60000-69999','70000-79999','80000-89999','90000-99999']:
		...
		for key in ModularSymbolsfile.keys():
			for prime in ModularSymbolsfile[key].keys():
				...
				if LeftSide % jajaj != RightSide % jajaj and LeftSide % jajaj != - RightSide % jajaj:
					good_primes = jajaj
					file.write(cremona_to_lmfdb(str(EllipticCurveSage.cremona_label())) + ";" + str(jajaj) + ';' + str(LeftSide % good_primes) + ';' + str(RightSide % good_primes) + ';' + str(CremonaEllipticCurve['torsion_order']) + ';' +'${}\\cdot{}^{} + O({}^{})$'.format(torsion, good_primes, meh, good_primes, meh + 1) + ';' + str(EllipticCurveSage.modular_degree()) + ';' + str(CremonaEllipticCurve['db_extra'][0]) + ';' + str(Integer(CremonaEllipticCurve['db_extra'][-1])) + '\n')
